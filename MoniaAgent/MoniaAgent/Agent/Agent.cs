                ...
                for (int turn = 0; turn < maxTurns; turn++)
                {
                    ...
                    if (completion.FinishReason == ChatFinishReason.ToolCalls)
                    {
                        ...
                        foreach (var toolCall in toolCalls)
                        {
                            ...
                            var truncatedArgs = argsString.Length > 40 ? argsString.Substring(0, 40) + "..." : argsString;
                            logger.LogInformation("[TOOLCALL] {ToolName}({Arguments}) --> {Result}", toolCall.Name, truncatedArgs, FormatActionOutput(toolResult));
                    ...
                    else
                    {
                        ...
                        if (consecutiveNonToolMessages >= 3)
                        {
                            break;
                        }
                    }
